...
class Board:
    ...
    @property
    def reward(self):
        rounds_cleared = max(0, self.i - 6)
        balls_gained = len(self.balls) - 1
        tile_sum = sum([t.c for t in self.tiles])
        return (rounds_cleared * 50 + balls_gained * 20 - tile_sum) / 1000
    
    @property
    def state(self):
        arr = np.empty((2, HEIGHT, WIDTH))
    ...
    def reset(self):
        ...
        if not self.headless:
            self.show_this = True
        elif self.show_every == 0:
            self.show_this = False
        else:
        ...
        if not self.show_this:
            ...
            self.video = cv2.VideoWriter(f'anim/generation_{self.resets}.mp4', fourcc, 60, (W, H))
